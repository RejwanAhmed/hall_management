...
<?php
    function get_row_count()
    {
        ...
        $sql = "SELECT COUNT(`id`) as total_row FROM `incoming_resource` WHERE `number_of_resources`!='0'";
    ...
    function display_content($run,$offset,$total)
    {
        ...
        if(empty($run))
        {
            $select_incoming_resource_qry = "SELECT r.resource_name, i.id, i.number_of_resources,i.entry_date FROM resource_name as r INNER JOIN incoming_resource as i ON r.id = i.resource_id WHERE `number_of_resources`!='0' LIMIT $offset, $total";
            $run = mysqli_query($conn,$select_incoming_resource_qry);
        }
        ?>
        <table class = "table table-bordered table-hover text-center table-responsive-lg">
            ...
            while($row = mysqli_fetch_assoc($run))
            {
                ...
                <tr>
                    ...
                    <td>
                        <a class = "btn btn_color" href="update_incoming_resource_information.php?id=<?php echo $row['id']?>"><b><span><i class="fas fa-edit "></i></span> Modify</b></a>
                    </td>
                    <td>
                ...
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-lg-6 col-md-6 col-12">
                            <h3 class ="text-center card_header_color text-uppercase">Incoming Resource <span><i class="fas fa-info-circle"></i></span></h3>
                        </div>
                        <div class="col-lg-3 col-md-3 col-12 text-center">
                            <a href="add_incoming_resource.php" class = "btn btn-danger"><b><span><i class="far fa-plus-square"></i></span> Resource</b></a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    if(isset($_POST['search']))
                    {
                        ...
                        $search_qry = "SELECT r.resource_name, i.id, i.number_of_resources,i.entry_date FROM resource_name as r INNER JOIN incoming_resource as i ON r.id = i.resource_id WHERE ";
                        $count=1;
                        if($resource!=NULL)
                        {
                            $count++;
                    ...
                    else
                    {
                        ...
                        <div class="col-lg-12 col-12">
                            <?php
                            $run = 0;
                            $page_name = 'incoming_resource_information';
...
<script>
    function deleteConfirmation(id)
    {
        ...
        if(del == true)
        {
            window.location='delete_incoming_resource_information.php?id='+id;
        }
    }
</script>
