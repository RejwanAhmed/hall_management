...
<?php
    function get_row_count()
    {
        ...
        $sql = "SELECT COUNT(DISTINCT(`year`)) as total_row FROM `expense_information` ";
    ...
    function display_content($run,$offset,$total)
    {
        ...
        if(empty($run))
        {
            $expense_information_qry_year_wise = "SELECT * , SUM(`amount`) FROM `expense_information` GROUP BY `year` ORDER BY `year` DESC LIMIT $offset, $total";
            $run = mysqli_query($conn, $expense_information_qry_year_wise);
        }
        ?>
        <table class = "table table-bordered table-hover text-center table-responsive-lg">
            ...
            while($row = mysqli_fetch_assoc($run))
            {
                ...
                <tr>
                    ...
                    <td>
                        <a class = "btn btn_color" href="expense_monthly_details.php?year=<?php echo $row['year']?>"><b><span><i class="fas fa-calendar-week"></i></span> Monthly Details</b></a>
                    </td>
                </tr>
                <?php
            }
                ...
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-12">
                            <h3 class ="text-center card_header_color text-uppercase">Expense Information Year Wise<span><i class="fas fa-info-circle"></i></span></h3>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    if(isset($_POST['search']))
                    {
                        ...
                        if($year!=NULL)
                        {
                            $search_qry = "SELECT * , SUM(`amount`) FROM `expense_information` WHERE `year` = '$year' GROUP BY `year` ORDER BY `year` DESC ";
                        }
                        else
                        {
                            ?>
